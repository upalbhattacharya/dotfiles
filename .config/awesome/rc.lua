...
-- Borders when rearranging only 1 non-floating but not maximized client
...
-- Move clients to the correct tags across screens
...
move_tag_clients_to_screen = function(cl, sc)
        local index=0
        local tag=0
        local present_tag_index=cl.first_tag.index
        for _, c in ipairs(client.get()) do
            index=c.first_tag.index
            if index == present_tag_index then
                c:move_to_screen(sc.index+1)
                tag =c.screen.tags[index]
                c:move_to_tag(tag)
                if tag then
                    tag:view_only()
                end
...
globalkeys = mytable.join(
    ...
    awful.key({ altkey }, "p", function() os.execute("flameshot screen") end,
...
clientkeys = mytable.join(
   ...
   -- working toggle titlebar
    ...
    awful.key({ modkey, "Control"   }, "o",      function () cycle_screens()
                                                                                             end,
              {description = "swap screens (hacky)", group = "client"}),

    awful.key({ modkey, "Shift"   }, "o",      function (c) move_tag_clients_to_screen(c, c.screen)
                                                                                             end,
              {description = "move all clients of active client's tag to screen", group = "client"}),

    awful.key({ modkey,           }, "o",      function (c) move_client_to_screen(c, c.screen.index+1)
                                                                                             end,
...
awful.rules.rules = {
    ...
    { rule_any = { name = {"ClickUp", "Google Calendar", "Todoist", "Habitify", "Toggl Track", "ActivityWatch" } },
    ...
    { rule_any = { name = { "Bitwarden", "Zotero", "YouTube Music", "Kindle", "Google Podcasts", "Tandoor Recipes" } },
      properties = {tag = "Misc" } },
}
...
-- xrandr
awful.util.spawn("xrandr --output HDMI-1 --auto --right-of eDP-1")
